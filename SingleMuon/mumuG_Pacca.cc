...
void mumuG_Pacca( const string& file,  const string& outfile ) {
...
//
	...
	TTreeReaderArray<Bool_t>  Muon_looseId          = {myread, "Muon_looseId" };
	TTreeReaderArray<Bool_t>  Photon_mvaID_WP80     = {myread, "Photon_mvaID_WP80"};
    TTreeReaderArray<float_t> Muon_pfRelIso03_all   = {myread, "Muon_pfRelIso03_all"};
    TTreeReaderArray<float_t> Photon_pfRelIso03_all = {myread, "Photon_pfRelIso03_all"};
    ...
    for (Long64_t i=0;i<oldtree->GetEntries(); i++) {
        ...
        //bool trigger quality   
        if (Muon_looseId[0]==false) save=false;
        if (Muon_looseId[1]==false) save=false;  
