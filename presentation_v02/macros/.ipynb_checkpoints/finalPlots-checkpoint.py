...
def plot_all_distr_SingleMuon(sigs, h_mu, plot_arrow=False, bound=2, ly=-1.5, hw1=0.1, hw2=15, hl1=15, hl2=0.1):
    ...
    fig, axs = plt.subplots(1,2, figsize=(18,6))
    ax0 = axs[0].hist2d(h_mu.flatten(), data_AllMu.flatten(),  range= [[-1, 401], [0, 100]], bins=(201, 120), cmap='inferno')
    axs[0].set_xlabel("$\mu$")
    axs[0].set_ylabel("q($\mu$)")
    axs[0].set_title("Distribution dependence on $\mu$")
    fig.colorbar(ax0[3], ax=axs[0])
    axs[1].plot(xs, sigs, lw=2)
    if plot_arrow: axs[1].arrow(xs[0], 2., x-xs[0], 0, length_includes_head=True, head_width=hw1, lw=2, head_length=hl1, color="darkorange")
    if plot_arrow: axs[1].arrow(x,     2., 0,      ly, length_includes_head=True, head_width=hw2, lw=2, head_length=hl2, color="darkorange")
    axs[1].set_xlabel("$\mu$")
    axs[1].set_ylabel("$\sigma_{med}(\mu)$")
    axs[1].set_title("Significance dependence on $\mu$")
    if plot_arrow: return x
