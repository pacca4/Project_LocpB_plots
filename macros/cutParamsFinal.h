class cutParams
{
    public: //dichiaro pubbliche le costanti così posso accedere direttamente
        //static const double massMuMuG_min; // cm/ps
        //static const double massMuMuG_max;
        //static const double massJPsi_diff;
        //static const double muonPt0_min;
        //static const double muonPt1_min;
        //static const double photonPt_min;
        //static const double JPsiGAngle_min;
        //static const double JPsiGAngle_max;
        //static const double MuMuAngle_min;
        //static const double MuMuAngle_max;
        //static const double photonEta_max;
        //static const double JpsiGdR_min;
        //static const double muGdR_min;
        //static const double JpsiGdPhi_min;

        static const double JPsiMass_min;
        static const double JPsiMass_max;
        static const double Pht_pt_min;
        static const double PhPt_RMass_min;
};


//const double cutParams::massMuMuG_min  = 70;
//const double cutParams::massMuMuG_max  = 150;
//const double cutParams::massJPsi_diff  = 0.09;
//const double cutParams::muonPt0_min    = 25;
//const double cutParams::muonPt1_min    = 10;
//const double cutParams::photonPt_min   = 33;
//const double cutParams::JPsiGAngle_min = -1;
//const double cutParams::JPsiGAngle_max = 0.95;
//const double cutParams::MuMuAngle_min  = 0.95;
//const double cutParams::photonEta_max  = 2.5;
//const double cutParams::MuMuAngle_max  = 1;
//const double cutParams::muGdR_min      = 1;
//const double cutParams::JpsiGdR_min    = 2;
//const double cutParams::JpsiGdPhi_min  = 1.5;

const double cutParams::JPsiMass_min   = 3.0;
const double cutParams::JPsiMass_max   = 3.2;
const double cutParams::Pht_pt_min     = 25;
const double cutParams::PhPt_RMass_min = 0.38;