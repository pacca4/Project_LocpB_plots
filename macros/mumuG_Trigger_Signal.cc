...
void mumuG_Trigger_Signal( const string& file,  const string& outfile ) {
/*
...
*/
	...
	TTreeReaderArray<Float_t> mupt  = {myread, "Muon_pt" };
	TTreeReaderArray<Float_t> phpt  = {myread, "Photon_pt" };
	...
	for (Long64_t i=0;i<oldtree->GetEntries(); i++) {
			...
			if (mupt[0]>20 && mupt[1]>7 && phpt[0]>25) {
				oldtree->GetEntry(i); newtree_2MuPh  ->Fill(); }
	}
